...
for i=1:5:size(t_laser,1)
    ...
    laser = [   x_turtle;   y_turtle;   ones(1, npoints)];
    ...
    x_mat = (x_interp(i)-ogxmin)/ogres;
    y_mat = (y_interp(i)-ogymin)/ogres;
    l_x =   (laser(1, :)-ogxmin)/ogres;
    l_y =   (laser(2, :)-ogymin)/ogres;
    l_x(isnan(l_x)) =   [];
    l_y(isnan(l_y)) =   [];
    ...
    for n = 1:size(l_x, 2)
        ...
        x_line =    min(300, round(linspace(x_mat, l_x(n), resol)));
        y_line =    min(180, round(linspace(y_mat, l_y(n), resol)));

        % UPdate free space
        for m = 1:resol
            oglo(y_line(m), x_line(m)) = oglo(y_line(m), x_line(m)) - 1;
        end
        
        % Update obstacle grids
        oglo(y_line(m), x_line(m)) = oglo(y_line(m), x_line(m)) + 5;
    end
