...
train_CL_list = zeros(1, num_epochs);
valid_CL_list = zeros(1, num_epochs);
...
for epoch = 1:num_epochs
  ...
  train_CL = 1 - sum(~xor( round(prediction), target_train ))/length(target_train);
  ...
  valid_CL = 1 - sum(~xor( round(prediction), target_valid ))/length(target_valid);
  ...
  train_CL_list(1, epoch) = train_CL;
  valid_CL_list(1, epoch) = valid_CL;
...
train_class(1, start_epoch : total_epochs) = train_CL_list;
valid_class(1, start_epoch : total_epochs) = valid_CL_list;
  figure;
  hold on, ...
  plot(epochs(1, 1 : total_epochs), train_class(1, 1 : total_epochs), 'b'),...
  plot(epochs(1, 1 : total_epochs), valid_class(1, 1 : total_epochs), 'g'),...
  legend('Train', 'Validation'),...
  title('Class Error vs Epoch'), ...
  xlabel('Epoch'), ...
  ylabel('Class Error');
  hold off;
