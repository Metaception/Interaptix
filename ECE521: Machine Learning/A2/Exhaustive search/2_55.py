...
class fully_connected_network:
	# global parameters
	output_classes = 10
	learning_rate = 0.009581697823359981 # should try with 3 different values
	weight_decay_factor = 0.001504707273836058
	...
	def __init__(self, inputs, targets, dropout):
		...
		self.hidden1_units = 288
		self.hidden2_units = 193
		self.hidden3_units = 379
		self.hidden4_units = 231
		self.hidden5_units = 157
		self.forward_prop()
		self.backward_prop()
	# Forward propagation
	def forward_prop(self, output_amount=output_classes):
		...
		# hidden_layer 4 to hidden layer 5
		with tf.variable_scope("hidden4_to_hidden5"):
			hidden5_z, hidden5_weights = layer_wise_activation_computation(self.hidden4, self.hidden5_units)
			self.hidden5 = tf.nn.relu(hidden5_z)
			self.hidden5_weights = hidden5_weights
			if(self.dropout[0] == 1):
				# only introduce dropout at the training time
				self.hidden = tf.nn.dropout(self.hidden5, keep_prob=0.5)
		# From hidden layer to output
		with tf.variable_scope("hidden5_to_output"):
			output_z, output_weights = layer_wise_activation_computation(self.hidden5, output_amount)
	...
	def backward_prop(self, l_rate = learning_rate, output_amount=output_classes, weight_decay_factor=weight_decay_factor):
		...
		self.weight_loss = (tf.nn.l2_loss(self.hidden1_weights) + \
						...
						tf.nn.l2_loss(self.hidden5_weights) + \
