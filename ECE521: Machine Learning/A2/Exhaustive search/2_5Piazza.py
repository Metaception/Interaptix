...
class fully_connected_network:
	# global parameters
	output_classes = 10
	learning_rate = 0.00383297 # should try with 3 different values
	weight_decay_factor = 0.00039561
	...
	def __init__(self, inputs, targets, dropout):
		...
		self.hidden1_units = 299
		self.hidden2_units = 326
		self.hidden3_units = 210
		self.forward_prop()
		self.backward_prop()
	# Forward propagation
	def forward_prop(self, output_amount=output_classes):
		...
		with tf.variable_scope("input_to_hidden1"):
			...
			if(self.dropout[0] == 1):
				...
				self.hidden1 = tf.nn.dropout(self.hidden1, keep_prob=0.74)
		...
		with tf.variable_scope("hidden1_to_hidden2"):
			...
			if(self.dropout[0] == 1):
				# only introduce dropout at the training time
				self.hidden = tf.nn.dropout(self.hidden2, keep_prob=0.74)
		...
		with tf.variable_scope("hidden2_to_hidden3"):
			...
			self.hidden3 = tf.nn.relu(hidden3_z)
			self.hidden3_weights = hidden3_weights
			if(self.dropout[0] == 1):
				# only introduce dropout at the training time
				self.hidden = tf.nn.dropout(self.hidden3, keep_prob=0.74)
		# From hidden layer to output
		with tf.variable_scope("hidden3_to_output"):
			output_z, output_weights = layer_wise_activation_computation(self.hidden3, output_amount)
