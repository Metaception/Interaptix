...
class fully_connected_network:
	...
	def forward_prop(self, output_amount=output_classes):
		...
		with tf.variable_scope("hidden2_to_output"):
			output_z, output_weights = layer_wise_activation_computation(self.hidden2, output_amount)
	...
	def backward_prop(self, l_rate = learning_rate, output_amount=output_classes, weight_decay_factor=weight_decay_factor):
		...
		self.weight_loss = (tf.nn.l2_loss(self.hidden1_weights) + tf.nn.l2_loss(self.hidden2_weights) + \
...
if(__name__=='__main__'):
	...
	with tf.Session() as sess:
		...
		plt.title('Cross Entropy for 100 Units')
		...
		plt.title('Classification Accuracy for 100 Units')
		plt.legend(loc='upper right')
		...
		print(np.average(class_valid_avg[-1]))
		print(np.average(correct_pred))
