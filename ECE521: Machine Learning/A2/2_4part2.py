...
class fully_connected_network:
	...
	def forward_prop(self, output_amount=output_classes):
		...
		with tf.variable_scope("input_to_hidden"):
			hidden_z, hidden_weights = layer_wise_activation_computation(self.inputs_flatten, self.hidden_units)
		...
		with tf.variable_scope("hidden_to_output"):
			output_z, output_weights = layer_wise_activation_computation(self.hidden, output_amount)
	...
	def backward_prop(self, l_rate = learning_rate, output_amount=output_classes, weight_decay_factor=weight_decay_factor):
		...
		self.weight_loss = (tf.nn.l2_loss(self.hidden_weights) + tf.nn.l2_loss(self.output_weights)) * weight_decay_factor
...
if(__name__=='__main__'):
	...
	with tf.Session() as sess:
		...
		for i in range(epoch_amount):
			...
			if( ((float(i+1)/epoch_amount) % 0.25) == 0 and i != 0):
